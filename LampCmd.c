/** Send lamp command, bridge initialization
    Author: Amy
*/
#include "LampCmd.h"
...
#define EMPTY_STR ""						// Empty string returned when error occurs
...
// Flag strings used to spit string.
...
/******************************  HTTP POST/PUT request format
<Method> /api/<url> HTTP/1.1
Host: xxx.xxx.xxx.xxx
Content-Length: <length>
Content-Type: text/plain;charset=UTF-8

//Notice: Method "GET" don't need Content fields and Json strings
<Json cmd string>
*/
...
// Remember to free request memory
...
// Remember to free response memory
...
int getUserName(char* deviceType)
{
    ...
    while((strstr(response, USER_NAME_HEAD_FLAG) == NULL) || (strstr(response, PRESS_BTN_ERROR) != NULL)
          || (countTime >= WAIT_PUSH_BTN_TIME))
    {
        free(response);
